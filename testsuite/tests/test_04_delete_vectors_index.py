...
class TestDeleteVectorsAndIndexes:
    @classmethod
    def setup_class(cls):
        """Initialize VectorX client, encryption key, and test indexes."""
        ...
        cls.test_indexes = [{"name": c["name"], "encrypted": c["encryption"]} for c in cls.index_configs]
        ...
        if not cls.pipeline_mode:
            ...
            existing_indexes = cls.vx.list_indexes().get('indixes', [])
            ...
            for config in cls.index_configs:
                ...
                cls.vx.create_index(**kwargs)
                logger.info(f"Created index: {config['name']}")

                index = cls.vx.get_index(config["name"], key=cls.encryption_key if config["encryption"] else None)
                num_vectors = 2000
    ...
    def teardown_class(cls):
        ...
        if cls.pipeline_mode:
            logger.info("Pipeline mode cleanup: Deleting test indexes.")
    ...
    def get_test_index(self, name, encrypted):
        """Fetch index with optional encryption key."""
        try:
